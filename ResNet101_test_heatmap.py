...
def getBatchDataset2(dataset, batch=64):
    dataset = dataset.map(read_and_decode, num_parallel_calls=tf.data.experimental.AUTOTUNE).repeat(1).shuffle(buffer_size=32000)
    dataset = dataset.map(lambda x, y: (standardize(x), y), num_parallel_calls=tf.data.experimental.AUTOTUNE).batch(batch_size=batch)
    dataset = dataset.prefetch(buffer_size=tf.data.experimental.AUTOTUNE)
    return dataset
...
# 加载上次结束训练时的权重
model.load_weights(r"doorModels\cp-030-0.014-0.995-1.000-0.995-0.261-0.941-0.953-0.939.h5")
...
print("heatmap:", heatmap)
# 选择阈值
heatmap = tf.where(heatmap > 0.4, heatmap, 0)
print("heatmap:", heatmap)
